...
@sensors.route('/sensors_select', methods=['POST'])
@login_required
def select_sensor():
...
@sensors.route('/create_sensor',  methods=['POST'])
@login_required
def create_sensor():
    ...
    if(name):
        response = Sensor.insert_sensor(user.id_user, name, model, brand, measure, voltage)
